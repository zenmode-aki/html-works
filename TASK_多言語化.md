...
- すでにある言語：**en（元）/ ja / ko / zh / zh-Hant**。記事は全158本
- ファイルの置き場所
...
| フェーズ | 言語（コード） | 注意すること |
|---|---|---|
| **A. 土台づくり** | （言語は足さない） | §3 のメニューと、トップページの軽量化を先に作る |
| **B. 左から書く・ふつうの文字** | es, fr, de, pt, it, nl, pl, sv, nb, da, fi, cs, ro, ru, uk, tr, el, id, ms, vi, fil, ceb, sw | いちばん簡単。まずここ |
| **C. タイ文字・インドの文字** | th, hi, bn, ta, te, mr, pa, ne | 端末にフォントがないことがある。§5 のフォントを足す |
| **D. 右から書く言語** | ar, fa, ur, he | レイアウトの左右反転が要る。§6 を必ず読む |

## 3. フェーズA：言語メニューを作り直す（いちばん大事）

40言語を国旗で並べると「設定画面か」というほど長くなって、読者がそこで挫折する。次の形にする。
...
- 検索欄は、その言語自身の名前・英語名・言語コードのどれでもヒットする（`ger` でも `deu` でも `de` でも Deutsch が出る）
- **国旗は使わない。** 国旗は国の印で、言語の印ではない（スペイン語は🇪🇸？🇲🇽？ アラビア語は20か国以上）。`i18n/ui.<lang>.json` に `flag` があっても表示しない
- ボタン自体は `🌐 English ▾` のように、地球マーク＋今の言語名だけ
- スマホ（幅360px）で、メニューが画面からはみ出さないこと。長いときはメニューの中だけスクロール
- キーボードでも使えること（Tab・矢印・Enter・Esc）

### 3-3. トップページを軽くする

- 今は**トップページに全言語の訳を埋め込んでいて**、1言語あたり約25KB。40言語だと約1.3MBになって重い
- トップページだけ、**選んだ言語の訳を後から読み込む**形にする（例：`i18n/top.<lang>.json` を公開フォルダに置いて `fetch`）。読み込むまでは英語のまま表示されていればよい
...
- `README.md` の「🌐 表示言語の切り替え」の表と説明を、新しい言語とメニューの形に合わせて更新する
- `CLAUDE.md` / `AGENTS.md` の「🌏 言語を増やす方針」の「いまある言語」を更新する
- `BACKLOG.md` から終わったものを消す
- 1フェーズごとに commit・push してよい（公開サイトなので、push するとすぐ本番に出る）
