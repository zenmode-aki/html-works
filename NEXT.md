...
## 🗾 トップの地図：Japan をクリックしたときの動きを変える（2026-09-11 に本人が指示）

**まだ実装していない。次にトップページを触るときにやる。**

### いまの動き（これをやめたい）

トップの地図で `Japan` をクリックすると、
**その下に記事の一覧がスライドして出てくる。**

> 「その機能いらなくて」

### やりたい動き

`Japan` をクリックしたら、**日本地図がふわっと出てくる**ようにする。

- 記事一覧を下にスライドさせるのはやめる
- 代わりに日本地図を表示する（ふわっと出す＝やわらかいフェード／スケールで）

### 触る場所

- `index.html` の `.place` / `#places`（276〜290行目あたりに CSS、820行目に container）
- 一覧の差し込みは `PLACES:START` 〜 `PLACES:END`。
  ここは `python3 tools/build-site.py` が `meta.json` から生成するので**手で書かない**
- クリックしたときの挙動のスクリプトを差し替えることになる

### 未確認（実装前に本人に聞く）

- 日本地図は「どこに住んでいたか・どこの話か」をピンで示すもの？　それとも飾り？
- 地図を出したあと、記事一覧はどこに出す？（出さない？　地図のピンから飛ぶ？）
