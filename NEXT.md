...
最終更新：2026-09-24（「あとで記事にするもの」6本と「ありがとうを言いたいもの」01を公開）
...
## 📥「あとで記事にするもの」6本＋エアコンを公開した（2026-09-24）

seq 135〜141。英語本文から日本語・韓国語の訳もつけてある。

| seq | slug | 元の下書き |
|---|---|---|
| 135 | `rules-of-this-blog` | ブログの投稿方針（英語は本人の 本文_🇬🇧.md をそのまま使用） |
| 136 | `spotify-dj-button` | SpotifyのDJ機能 |
| 137 | `elevator-door-ad-screen` | エレベーターの扉の広告スクリーン |
| 138 | `trampoline-gym-45-minutes` | ジャンプワンのトランポリンジム（曲は Owl City「Fireflies」を Spotify カードで） |
| 139 | `chinese-taipei-house-in-nagoya` | 名古屋のチャイニーズタイペイハウス |
| 140 | `asian-games-monument-in-sakae` | 栄のアジア大会モニュメント |
| 141 | `thank-you-air-conditioner` | ありがとうを言いたいもの/01 エアコン（英語は本人の 🇬🇧 版そのまま） |

- 公開した6本の下書きは「あとで記事にするもの」から消した（git の履歴には残っている）
- 写真の匿名化：エレベーターの広告に写っていた人の顔、TEAM TPE の写真の車のナンバーにモザイク
- 表紙7枚＋途中の画像1枚は z_image で生成（0.15クレジット×8枚）
- `date` は下書きを書いた日（09-11 / 09-19）。並び順は seq で決まる

---

## 🔢「Jobs he has had」の数字が合っていなかったのを直した（2026-09-12）

**症状：** 💼 を開いて「Bridge engineer 22 posts」をクリックすると、
記事が2本しか出てこない。💼 の数字も37本になっていた（実際は68本あった）。

**原因：** `index.html` の `ROOM_OF`（どの記事がどの部屋か）を**手で書いていた**ため、
ブリッジSE14本・ネットワーク運用保守7本など**31本が書き足されないまま**古くなっていた。
記事一覧は「部屋 AND 仕事」の両方で絞るので（`drawFeed()`）、
ROOM_OF に載っていない記事は、仕事を選んだ瞬間に消えていた。

**直したこと：**

1. 💼 jobs の部屋は `ROOM_OF` ではなく **`JOBS` から決める**ようにした
   （`roomHas()` / `inAnyJob()`）。これでもう古くならない
2. `ROOM_OF` は `build-site.py` が **meta.json の `"room"` から生成**するようにした
   （`/* ⬇️ ROOM_OF:START ⬇️ */` 〜 `/* ⬆️ ROOM_OF:END ⬆️ */`）
3. 全記事に `room` を割り当てた。**仕事の記事には `room` を書かない**
   （JOBS から自動で 💼 に入るため）
4. ROOM_OF が指していた存在しない記事（`ebinpu-new-perspective`）を削除
5. 仕事の話なのに topic がズレていた2本を `netops` に訂正
   （`open-tomorrow-manuals`: life →、`links-keep-work-moving`: ai →）
6. `check.py --site` に「部屋が決まっていない記事」の検査を追加

**新しい記事を作るときにやること：** 仕事の記事以外は meta.json に
`"room"`（`head` / `keep` / `likes` / `wants` / `oops`）を1つ足す。忘れても
`tools/check.py --site` が止めてくれる。

---

## 🏫 クラーク・バギオシリーズを新設して、18本を公開した（2026-09-12）
...
- **クラーク 12本** — `クラーク/記事/01〜12` → 💼 Language school（Clark）
- **バギオ 4本** — `バギオ/記事/01〜04` → 💼 English student（Baguio、既存1本とあわせて5本）
- **一本もの 2本** — `one-bite-hamburger`（フィリピンのマクドナルド）、
  `bulky-trash-once-a-month`（名古屋の粗大ゴミ）

slug の対応表は `クラーク/情報源.txt` と `バギオ/情報源.txt` の末尾にある。
topic は全部 `ph`（一本ものの粗大ゴミだけ `life`）。
`PLACES` に `clark` を足し、`LEGACY_JOB_POSTS` に `clark` と `baguio` を書いた。

### 画像について

- 表紙18枚は `z_image` で生成（0.15クレジット/枚・合計約2.9クレジット）
- 本人が撮った写真6枚は記事の中に置いた（表紙は生成画像のまま）
- `one-bite-hamburger` は1枚目に**背景で小さいペンギンが棚に並んでしまった**ので、
  「plain empty background・他の物を置かない」と書き足して作り直した（集合体恐怖症への配慮）
- クラークの受付写真は、学校名らしきサインと教会の電話番号が写っていた右半分を
  切り落としてから使っている

### 本人が決めた、フォルダの役割分担（2026-09-12）

- **「あとで記事にするもの」＝ スマホからGitHubに下書きを置くための一時置き場だけ。**
  記事にして公開したら、ここからは消す
- **これから何本も増えるシリーズは、ここに置かずトップレベルの独立フォルダにする**
  （`クラーク/` `バギオ/` `ブリッジSE/` `ネットワーク運用保守/` … と同じ形）
- 詳しくは `あとで記事にするもの/README.md`
...
- **岐阜ドライブの記事**：母の体調のことは、2026-09-26 に病名を出さない書き方に変えた（家族の健康情報のため）
...
### ④ ブリッジエンジニアの記事のタグ分けを見直す 🔴（2026-09-12・あきくんの指摘）

あきくん本人から：「ブリッジエンジニアの記事が、タグ付けというカテゴリー分けがうまく
できていないから、そこを直しておいて」という指摘。**どこがどう変か、具体的な中身は
まだ聞けていません。** 次にあきくんと話すときに詳しく聞くこと。

いまの分け方（参考、下の「🌉 ブリッジSEシリーズ19本を出しました」も見てください）：

- `works/*/meta.json` の `topic` が `bridge`（仕事そのものの話・14本）か
  `ph`（セブ暮らしの話・5本）のどちらか1つだけ入っている
- セブ暮らしの5本（`topic: "ph"`）は `tools/build-site.py` の
  `LEGACY_JOB_POSTS["cebu"]` にも重複して足してあり、💻 Bridge engineer の
  仕事の一覧にも並ぶようにしてある（暮らしていた時期＝ブリッジSEの時期のため）
- トップページのバッジは `topic` が `bridge` か `netops` のときだけ出る
  （他のtopicにはバッジを出さない設計。本人の要望でスコープを絞った）

やること：
1. あきくんに「どのあたりが分かりにくいか」を具体的に聞く
   （bridge/phの線引き？バッジの見た目？記事の並び方？）
2. 聞いた内容に沿って `works/*/meta.json` の `topic`（必要なら新しいタグ体系）を直す
3. `tools/build-site.py` の該当ロジック（`LEGACY_JOB_POSTS`、バッジ判定）も合わせて直す
...
> 「その記事に入ってる文章は、その文章量でいいと思ったから私がそうやって決めてるわけで、
