...
## 🌐 表示言語の切り替え（2026-09-23）

**英語の本文は1文字も変えずに、見る人が言語を選べるようにしている。**
右上の少し下にある `🌐 English ▾` で切り替える。選んだ言語は覚えておく（次の記事でもそのまま）。
初めて来た人は、ブラウザの言語で自動で決まる（日本語のブラウザなら日本語、韓国語のブラウザなら韓国語）。`?lang=ja` / `?lang=ko` でも指定できる。
...
i18n/ui.ja.json              ← 全記事に共通の言葉（← Back、分類ラベル、日本語版の一言）
i18n/ui.ko.json              ← 韓国語名・国旗・共通UI・未確認翻訳の注意書き
i18n/top.ja.json             ← トップページの言葉
i18n/top.ko.json             ← トップページの韓国語
tools/i18n_runtime.js        ← ブラウザで動く切り替えの本体
tools/i18n.py                ← 上の訳と本体を、各ページの </body> の手前に埋め込む
```

- **なぜページを言語ごとに複製しないのか**：写真が base64 で入っているので、複製すると容量が言語の数だけ倍になる。1ページの中で文だけ差し替える
- **英文を書き直すと、その文の訳は外れて英語のまま表示される**（壊れはしない）。`python3 tools/check.py --site` が ⚠️ で教えてくれる
- 次の記事のタイトル・トップの一覧のタイトルは、各記事の `title` から自動で入る
- 切り替えは国旗つきの横並び `[🇺🇸 English | 🇯🇵 日本語 | 🇰🇷 한국어]`。言語が4つ以上になったら、国旗つきのメニューに自動で変わる。
  国旗は `i18n/ui.<lang>.json` の `flag` に書く
- 韓国語に切り替えると、運営者が韓国語を読めず翻訳を確認できないことを知らせる注意書きが表示される。閉じるとそのブラウザでは再表示しない
- 「日本語は英語からの訳です」という説明は、**全記事には付けない**（毎回言い訳を出すのは逃げ腰に見えるので）。
  1本の記事として出す（`あとで記事にするもの/日本語版について/`）（2026-09-24 本人の判断）
- **その言語で見ている人にだけ出す記事**：`meta.json` に `"only": "ja"` と書くと、日本語で見ている人の一覧にだけ、
  いちばん上に出る（英語の一覧には出ない）。例：`works/a-note-for-japanese-readers/`（主人から日本語の読者への一言）
- 訳と切り替えは各ページの `<head>` に入る。言語はそこで先に決まり（`window.PENGESSO_LANG`）、
  文の差し替えはページを読み終わってから動く
