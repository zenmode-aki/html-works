...
| 長さ | 本文 **35〜55 words**（55 words ≒ 18秒） |
...
```
    ...
    毎朝 6:00   AIが昨日ぶんを全部読んで、15秒記事の下書きを作る（最大3本）
                 → drafts/<slug>/ に置く。GitHubには上げない
                    ↓
                #html-my-experience-idea に完成プレビューのリンクが飛ぶ
                    ↓
      あきくん   スマホで見る。よければ 👍 を付ける
                 直したいことがあれば、そのスレッドに書く
                    ↓
    毎晩 21:00   👍 が付いたものだけ works/ へ移動して push
                 → GitHub Pages に公開される
```

👍 を付けなかったものは `drafts/` に残るだけ。**勝手に公開されることはない。**

### スケジュールタスクの中身

| taskId | 時刻 | 何をするか |
|---|---|---|
| `blog-morning-cook` | 毎朝 6:00 | Slack12chを読む → 下書き生成 → Slackにプレビュー投稿 |
| `blog-publish-approved` | 毎晩 21:00 | 👍 の付いた下書きを公開 → push |
