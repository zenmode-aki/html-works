...
| 枚数 | **あきくんが写真を添付したら、その写真は必ず使う。生成画像も必ず1枚作る。**つまり添付1枚の記事は「生成した表紙1枚＋添付写真1枚＝2枚」になる（2026-09-06） |
...
| 画像 | **本人が渡した写真は1枚も落とさない。**渡された写真は全部使う。必ず base64 で埋め込み、相対パスも外部URLも使わない |
| 1記事のサイズ | 通常は **400KB以内**。複数写真のフォトストーリーは **1.5MB以内** |
| 見せ方 | 文章を少なく、写真・コード風パネル・図解・控えめなアニメーションを多めにする |
| 更新ペース | 毎朝AIが下書きを作る。気が向いたときにOKを出す |
| 公開 | **「ブログ作って」と言われたら、本番（push）まで必ずやる。**「出していい？」と聞かない。出したあとに直す（2026-09-06） |
| 匿名 | 通常は本名・顔・勤務先・学校名を出さない。ただし、本人がその素材の掲載を明示した場合は、その指定範囲だけ使用してよい |

---

## 📸 写真をもらったときの決まり（2026-09-06）

**あきくんが写真を添付したら、その写真は必ず記事に入れる。**
「合わなかったので使いませんでした」は無し。もらった写真は1枚も落とさない。

**それとは別に、生成画像も必ず1枚作る。** 表紙（1枚目）はAIで描いたペンギンで固定です。
一覧のサムネがバラバラにならないようにするため。

```
写真を1枚もらった 15秒記事
  1枚目  🐧 生成したペンギン（表紙・サムネにも使う）
  2枚目  📷 もらった写真
                                    ← 合計 2枚
```

写真を2枚もらったら3枚、3枚もらったら4枚になります。**生成1枚 ＋ もらった枚数。**

---

## 🚀 「ブログ作って」＝ 本番まで（2026-09-06）

**あきくんが記事を頼んだら、push して公開するところまでが1回の作業です。**

「作りました。出していいですか？」と聞かない。承認を待たない。下書きで止めない。
出す場所は1つしかないので、出してから直します。

```
素材をもらう → 記事を書く → 画像を作る → check.py → build-site.py → commit → push
                                                                       ↑ ここまでやって1回
```
...
| **整理** | `card-label` / `h1` | **本人の素材の要約・直訳だけ** | 数えない | 対象外 |
| **視覚演出** | 絵文字 / 画像 / CSS / アニメ | **AIが作ってよい** | 数えない | 対象外 |

**本人が言っていない文章は、ツッコミ・小言・感想を含めて追加禁止。**

この線は `tools/check.py` が機械的に守っている（`<p>` の中身しか数えない）。
AIが自由に作ってよいのは、画像・色・レイアウト・動きなど文字ではない演出だけ。
...
- **記事内の文章は本人のもの。AIが体験・感想・事実・小言・ツッコミを創作しない。**
  `card-label` と `h1` も本人の素材だけを整理・英訳する。AIが自由に作れるのは絵文字・画像・CSS・アニメ
