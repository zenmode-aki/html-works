...
| | |
|---|---|
| 言語 | **公開記事の基準言語は英語。** 日本語などの表示訳は、公開中の英語本文から直接作る。現在40言語に対応。未確認の機械翻訳には、その言語で注意書きを出す（下の「🌐 表示言語の切り替え」） |
| 原稿の優先言語 | 原稿は日本語で書くことがあるが、公開後の多言語翻訳の基準は英語本文。英語本文を更新したら、訳は英語から作り直す。日本語訳などから別言語へ二次翻訳しない |
| 一人称 | **日本語はどこでも「私」。**訳・原稿・添削・下書き・source.md すべて。本人の素材に「僕」があれば「私」に直して使う。例外は歌詞など他人の作品の引用だけ（2026-09-24 本人が全体のルールにした） |
| フォント | **英語は今までどおり（Arial Rounded などの端末のフォント）。日本語は Zen Maru Gothic、韓国語は Jua。** 端末に適切な字形がない言語は、選択時に対応する太字 Google Font を読み込む |
| 文字の太さ | **細い文字は使わない。本文も含めて太字。**本人は細い文字が全部嫌い（「全部太字でいいぐらい」2026-09-24）。`tools/i18n_runtime.js` が全言語・全ページで本文を太字にする |
| 前提知識 | **読者は日本に詳しくない海外の人。** 「東海豪雨」のような国内限定の固有名詞や、地名の知名度に頼らない。体験ベースで説明する（2026-09-10） |
| 長さ | **語数の制限なし（2026-09-10 に廃止）。** 素材の日本語を1文も落とさず全部訳す。短い素材は短い記事、長い素材は長い記事でいい（[PROMPT.md §4](PROMPT.md)） |
| タイトル | **キャッチーにしない。**読んだだけで中身がわかる説明文にする |
| 右上 | **PUBLIC** を必ず出す（出す場所は1つしかないので、全部これ） |
| 長さの表示 | ページ上部に `⚡ 47 words · 16 sec` を出す |
| カード | **枚数の上限なし。** 素材の文の数だけ並べる。1枚に1〜2文 |
...
| 拾い物 | 制限なし（2026-09-10 に本人がこのルールを撤廃） |
| タグ | 日本語原稿の末尾に `## タグ` で分類を書いてよい（例：`仕事での失敗経験`）。横断で読みたくなるジャンルにだけ付ける。英語版に実装するかは未定（2026-09-10） |
| AIの想像力 | **見た目は自由。文章も、読みやすくするためなら直してよい。** アニメ・色分け・図・比べる表・動くUIなど、HTMLでしかできない工夫をAIの判断でどんどん入れる。長い段落は意味を変えずに分ける・短く言い換える・見せ方を変えてよい（2026-09-24 本人が制限を外した。それまでは「文章には使わない」だった） |
| 画像の優先順位 | **可愛さより「パッと見で何の話か分かる」。** 記事の中身を示すモノを必ず1つ絵に入れる（2026-09-10） |
| 怖い画像 | **禁止。** 暗すぎ・不気味・鋭いもの・血はNG。動物も牙や鋭い目つきになったらやり直す（2026-09-10） |
| 人間とリアルさ | **人間は出さない**（本人が好まないため）。ただし**モノ・機械・道具・風景はリアルに描いてよい**。やわらかさをかけるのはペンギン本体だけ（2026-09-10） |
| 素材の集め方 | **ポジティブな質問から聞く。**「何を学んだ?」ではなく「何がいちばん楽しかった?」「どの日がよかった?」。反省の素材は、意味づけ（オチ）まで運んでから書く（2026-09-11、[PROMPT.md §2.6](PROMPT.md)） |
| 添削のしかた | **ポジティブな側から書く。** 原稿があまりに暗いときは、勝手に明るく書き換えず、**オチになる明るい面を本人に逆質問してから**書く。本人が「ない」と言えばそのまま出す（2026-09-11、[PROMPT.md §2.6](PROMPT.md)） |
| カテゴリー | **記事を開いた瞬間に何の話か分かるように、上にバッジを出す。** `meta.json` の `topic`（netops / bridge / ph / travel / study / ai / blog / life）から入る（2026-09-11、[PROMPT.md §2.5](PROMPT.md)） |
| 一覧の仕事バッジ | **トップページの一覧でも、netops / bridge の記事だけ**小さな色つきバッジ（🛠 Network Ops / 🌉 Bridge SE）を出す。タイトルだけでは「どの仕事の話か」分からないため（2026-09-11） |
| 一覧の並び順 | **meta.json の `"seq"`（公開した順の通し番号）で決まる。** `date` は表示用だけで並び順には使わない。新しい記事には既存の最大値+1を書くこと（2026-09-11、詳しくは [NEXT.md](NEXT.md)） |
| favicon | ペンゲッソがサングラスをかけた顔。`assets/favicon-32.png` / `favicon-180.png`。`_template/index.html` に入っているので新しい記事には自動で入る（2026-09-11） |
| 画像のタッチ | **リアル・3D寄りにする。** 平たいイラスト／カートゥーン風は「ダサい」。`realistic 3D render, studio lighting` 系の指定を入れる。お手本は `ip-phone-goes-sideways` の2枚目（2026-09-11） |
| 集合体 | **本人は集合体恐怖症。** 入れ子が続く絵、小さいものが壁一面に並ぶ背景、ペンギンのお腹の同心円はNG。背景はシンプルで奥行きのある空間に（2026-09-11） |
| パソコンの見た目 | **Mac で描く。** 実際の職場は Windows だが、macOS のほうが怖くないため。Windowsの青い画面や四角い窓の連なりは避ける（2026-09-11） |
| ペンギンの色 | **ピンクにしない。** 粘土の指紋・傷・ひびのような「傷ついて見える」質感も使わない（2026-09-11） |
| ガラス | **ガラスっぽい見た目は禁止。** ステンドグラス風の質感は質感リストから削除した。透明・ツヤツヤ・光が透ける系は使わない（2026-09-11） |
| 生成画像の枚数 | **1記事に2枚。** 1枚目＝表紙（必ずペンギン・一覧のサムネ兼用）、2枚目＝記事の真ん中あたり（**ペンギンでなくてよい**。出てくるモノをリアルに描いてOK）。本人の写真が2枚以上ある記事は生成は表紙1枚でよい（2026-09-11） |
| Part分割 | **Part 1 / 2 / 3 に分けない**（本人が「分けて」と言ったときだけ）。分けるときは2本目以降のいちばん上に `.prev` リンクを置いて、前の記事へ飛べるようにする（2026-09-11、[PROMPT.md §4](PROMPT.md)） |
| 地図 | 場所の話なら **OpenStreetMap を埋め込む**（上から見て雰囲気がわかるように） |
| 動画 | 本人のYouTubeがあれば **記事の中にそのまま埋め込む**（2026-09-12に変更）。`.video-card` ＋ `youtube-nocookie.com/embed/` の iframe を使う（[PROMPT.md §14](PROMPT.md)）。別タブへのカード＋リンクは古い作り方 |
| 画像生成のモデル | 好みの絵が出るのは `nano_banana_2`（2クレジット）。**残高が少ないときは `z_image`（0.15クレジット）で十分**。ブリッジSE19本ぶん37枚はこれで作った（2026-09-11） |
| 画像 | **本人が渡した写真は1枚も落とさない。**渡された写真は全部使う。必ず base64 で埋め込み、相対パスも外部URLも使わない |
| 1記事のサイズ | 通常は **1MB以内**。複数写真のフォトストーリーは **1.5MB以内** |
...
## 🌐 表示言語の切り替え（2026-09-25）

**英語の本文は1文字も変えずに、見る人が言語を選べるようにしている。**
初めて来た人はブラウザの言語から自動で選ばれます。手動で選んだ言語は次の記事でも覚えておきます。`?lang=ja` / `?lang=ko` のようにURLでも指定できます。公開中の記事は英語を基準にし、全177本を表示言語に合わせて切り替えます。

ボタンは、地球儀・現在の言語名・いくつかの小さな旗を重ねたデザインです。開くと「たぶんこれ」（端末の言語・English・日本語）と全言語の一覧が出ます。検索欄は置かず、英語名の頭文字 A〜Z のボタンで一覧内を移動します。一覧は各言語自身の名前と英語名を表示し、英語名のABC順に並べます。

```
works/<slug>/index.html       ← 英語の本文（ここはいつもどおり書く）
works/<slug>/i18n/ja.json     ← 日本語訳 {"title": "訳したタイトル", "text": {"英文": "訳"}}
works/<slug>/i18n/ko.json     ← 英語本文から直接作った韓国語訳
i18n/ui.<lang>.json            ← メニュー名・英語名・ブラウザ判定用別名・旗・共通UI・注意書き
i18n/top.<lang>.json           ← トップページの翻訳元データ
i18n/top-data.<lang>.json     ← ビルドで作る、トップ用の圧縮済み翻訳データ
tools/i18n_runtime.js          ← ブラウザで動く切り替えの本体
tools/i18n.py                  ← 訳を記事に埋め、トップ用データを生成する
```

- **なぜページを言語ごとに複製しないのか**：写真が base64 で入っているので、複製すると容量が言語の数だけ倍になります。記事ページの中で文だけ差し替えます
- **トップページは軽く保つ**：言語名やメニュー情報は埋め込み、トップの本文訳は選ばれた言語の `i18n/top-data.<lang>.json` だけを後から読み込みます。読み込み前は英語で表示されます。記事ページの訳は今までどおり埋め込みます
- **英文を書き直すと、その文の訳は外れて英語のまま表示される**（壊れはしません）。`python3 tools/check.py --site` が知らせます
- 次の記事のタイトル・トップの一覧のタイトルは、各記事の `title` から自動で入ります
- `i18n/ui.<lang>.json` の `name` はその言語自身の名前、`englishName` は英語名、`aliases` は端末言語判定に使う別名、`flag` はボタンの小さな旗に使います
- 現在の表示言語は40言語：English (`en`)、日本語 (`ja`)、한국어 (`ko`)、简体中文 (`zh`)、繁體中文 (`zh-Hant`)、العربية (`ar`)、Español (`es`)、Français (`fr`)、Deutsch (`de`)、Português (`pt`)、हिन्दी (`hi`)、Bahasa Indonesia (`id`)、Tiếng Việt (`vi`)、ไทย (`th`)、Filipino (`fil`)、Bahasa Melayu (`ms`)、বাংলা (`bn`)、اردو (`ur`)、Русский (`ru`)、Türkçe (`tr`)、فارسی (`fa`)、Nederlands (`nl`)、Italiano (`it`)、Polski (`pl`)、Svenska (`sv`)、Norsk (`nb`)、Dansk (`da`)、Suomi (`fi`)、मराठी (`mr`)、Čeština (`cs`)、Română (`ro`)、עברית (`he`)、ਪੰਜਾਬੀ (`pa`)、नेपाली (`ne`)、Ελληνικά (`el`)、Українська (`uk`)、Kiswahili (`sw`)、தமிழ் (`ta`)、తెలుగు (`te`)、Cebuano (`ceb`)
- フェーズBの23言語、フォントを切り替えるフェーズCの8言語、右から左へ書くフェーズD（`ar ur fa he`）まで対応。タミル語・テルグ語の記事中の歌詞2文は、引用の原文を守るため英語のまま表示します。RTLの4言語では、文字の方向とページ送りを右から左へ切り替えます。
- 韓国語・中国語と新しく追加した言語は、運営者が翻訳を確認できないことを各言語で知らせます。閉じると、その言語では同じブラウザに再表示しません
- **その言語で見ている人にだけ出す記事**：`meta.json` に `"only": "ja"` と書くと、日本語で見ている人の一覧にだけ、いちばん上に出ます（英語の一覧には出ません）。例：`works/a-note-for-japanese-readers/`
- 言語は `<head>` で先に決まり（`window.PENGESSO_LANG`）、文の差し替えはページを読み終わってから動きます
- 言語を足す手順：`i18n/ui.<lang>.json` と `i18n/top.<lang>.json` を作る → 全記事に `i18n/<lang>.json` を置く → `python3 tools/i18n.py`
...
7. 🌐 **公開中の英語本文から、現在対応している表示言語の訳をつける**（下の「🌐 表示言語の切り替え」）
...
```
□ 通常記事は1MB以内、複数写真のフォトストーリーは1.5MB以内か
...
- **フレームワーク・npm・ビルドを足さない**（素のHTML/CSS だけ）
  JavaScriptは、モーションキットの IntersectionObserver と、🌐 言語切り替え（`tools/i18n_runtime.js`。
  `tools/i18n.py` が埋め込む）だけ許可。外部ライブラリは禁止
