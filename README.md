...
| | |
|---|---|
| 言語 | **本文は英語で書く。表示は切り替えられる（🌐 English / 日本語）。** 日本語版は英語からの訳で、`works/<slug>/i18n/ja.json` に置く。中国語・韓国語・フランス語・ドイツ語も同じ形で足していく予定（2026-09-23、下の「🌐 表示言語の切り替え」） |
...
初めて来た人は、ブラウザの言語で自動で決まる（日本語のブラウザなら日本語）。`?lang=ja` でも指定できる。

```
works/<slug>/index.html      ← 英語の本文（ここはいつもどおり書く）
works/<slug>/i18n/ja.json    ← 日本語訳 {"title": "訳したタイトル", "text": {"英文": "訳"}}
works/<slug>/i18n/ko.json    ← 韓国語を足すときは、ここに置くだけ
...
- 切り替えは国旗つきの横並び `[🇺🇸 English | 🇯🇵 日本語]`。言語が4つ以上になったら、国旗つきのメニューに自動で変わる。
