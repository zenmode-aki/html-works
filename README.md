...
| | |
|---|---|
| 言語 | **本文は英語で書き、表示言語は English / 日本語 / 한국어 から選べる。** 日本語・韓国語とも公開中の英語本文から直接訳し、`works/<slug>/i18n/<lang>.json` に置く。韓国語は運営者が読めず正しさを確認できないため、韓国語表示のときだけ注意書きを出す。中国語・フランス語・ドイツ語も同じ形で追加予定（2026-09-23、下の「🌐 表示言語の切り替え」） |
| 原稿の優先言語 | **日本語が正。** 本人が直すのはほぼ日本語だけで、英語版の原稿は遅れていることが多い。公開するときは日本語の原稿から英訳し直し、英語版の原稿も更新してから push する（2026-09-24 本人が決めた） |
| 一人称 | **日本語はどこでも「私」。**訳・原稿・添削・下書き・source.md すべて。本人の素材に「僕」があれば「私」に直して使う。例外は歌詞など他人の作品の引用だけ（2026-09-24 本人が全体のルールにした） |
| フォント | **英語は今までどおり（Arial Rounded などの端末のフォント）。日本語は Zen Maru Gothic、韓国語は Jua（太くて丸い）**（やわらかい丸ゴシック。どの端末でも同じに見えるよう Google Fonts から読み込む）。日本語・韓国語表示のときだけ `tools/i18n_runtime.js` が読み込む（2026-09-24 本人の希望） |
...
初めて来た人はブラウザの言語から自動で選ばれます。手動で選んだ言語は次の記事でも覚えておきます。`?lang=ja` / `?lang=ko` のようにURLでも指定できます。

ボタンは、地球儀・現在の言語名・いくつかの小さな旗を重ねた、今のデザインを使います。開くと検索欄、「たぶんこれ」（端末の言語・English・日本語）、全言語の一覧が出ます。一覧は言語自身の名前と英語名で表示し、英語名のABC順に並べます。検索は名前・英語名・コード・別名に対応しています。
...
i18n/ui.<lang>.json            ← メニュー名・英語名・検索別名・旗・共通UI・注意書き
...
- `i18n/ui.<lang>.json` の `name` はその言語自身の名前、`englishName` は英語名、`aliases` は検索・端末言語判定に使う別名、`flag` はボタンの小さな旗に使います
- 現在の表示言語は English（元）・日本語・한국어・简体中文・繁體中文です
- 韓国語や中国語に切り替えると、運営者が読めず翻訳を確認できないことを知らせる注意書きが表示されます。閉じると、その言語では同じブラウザに再表示しません
...
7. 🌐 **英語本文から日本語・韓国語の訳をつける**（下の「🌐 表示言語の切り替え」）
