...
| | |
|---|---|
| 言語 | **公開記事の基準言語は英語。** 日本語などの表示訳は、公開中の英語本文から直接作る。現在28言語に対応。未確認の機械翻訳には、その言語で注意書きを出す（下の「🌐 表示言語の切り替え」） |
...
初めて来た人はブラウザの言語から自動で選ばれます。手動で選んだ言語は次の記事でも覚えておきます。`?lang=ja` / `?lang=ko` のようにURLでも指定できます。公開中の記事は英語を基準にし、全158本を表示言語に合わせて切り替えます。
...
- 現在の表示言語は28言語：English (`en`)、日本語 (`ja`)、한국어 (`ko`)、简体中文 (`zh`)、繁體中文 (`zh-Hant`)、Español (`es`)、Français (`fr`)、Deutsch (`de`)、Português (`pt`)、Bahasa Indonesia (`id`)、Tiếng Việt (`vi`)、Filipino (`fil`)、Bahasa Melayu (`ms`)、Русский (`ru`)、Türkçe (`tr`)、Nederlands (`nl`)、Italiano (`it`)、Polski (`pl`)、Svenska (`sv`)、Norsk (`nb`)、Dansk (`da`)、Suomi (`fi`)、Čeština (`cs`)、Română (`ro`)、Ελληνικά (`el`)、Українська (`uk`)、Kiswahili (`sw`)、Cebuano (`ceb`)
